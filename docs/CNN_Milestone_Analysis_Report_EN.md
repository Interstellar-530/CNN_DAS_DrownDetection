...
|---|---|---:|---:|---|---:|---:|---:|---:|---:|---:|
| **01** | Initial baseline: lightweight CNN, 4450 clips, severely imbalanced | 16 | 0.25 | macro-F1 | 0.377 | 0.491/0.349 | 0.56/0.25 | 0.41/0.55 | 0.138 | 0.063 |
| **02** | V2 architecture + FSD50K screams, GPU | 32 | 0.30 | scream-F1 | 0.520 | 0.524/0.512 | 0.73/0.95 | 0.58/0.50 | 0.475 | 0.294 |
| **03** | Tempering: macro-F1, stronger regularization | 32 | 0.40 | macro-F1 | 0.554 | 0.711/0.564 | 0.80/0.97 | 0.72/0.38 | 0.374 | 0.201 |
| **04** | Re-balanced dataset | 48 | 0.35 | accuracy | 0.589 | 0.650/0.600 | 0.76/0.97 | 0.76/0.42 | 0.406 | 0.228 |
| **05** | Aligned labels, balanced data | 48 | 0.50 | scream-F1 | 0.514 | 0.579/0.512 | 0.81/0.62 | 0.60/0.48 | 0.302 | 0.083 |
| **06** | Reference recipe + balanced data | 32 | 0.40 | macro-F1 | 0.520 | 0.615/0.515 | 0.78/0.78 | 0.65/0.47 | 0.362 | 0.099 |
| **07** | + 89 real FSD50K water clips | 48 | 0.50 | scream-F1 | 0.571 | 0.634/0.553 | 0.82/0.68 | 0.65/0.65 | 0.439 | 0.092 |
| **08** | + 55 fresh FSD50K dev screams (final) | 48 | 0.50 | scream-F1 | 0.531 | 0.656/0.515 | 0.76/0.88 | 0.57/0.57 | 0.496 | 0.135 |
| **09** | Dual-recall objective (min scream R, water R) | 48 | 0.50 | dual-recall | 0.543 | 0.548/0.508 | 0.58/0.97 | 0.59/0.70 | 0.682 | 0.383 |
| **10** | 08 recipe + 80 epochs | 48 | 0.50 | scream-F1 | 0.577 | 0.681/0.565 | 0.76/0.95 | 0.62/0.58 | 0.554 | 0.205 |
| **11** | Data cleaning + architecture-optimization retrain (late-stage) | 32 | 0.40 | macro-F1 | 0.663 | 0.785/0.672 | 0.97/0.95 | 0.84/0.52 | 0.491 | 0.063 |
| **12** | Probability fusion (0.5/0.5): water-strong + scream-strong | 48 | 0.50 | fusion | 0.583 | 0.663/0.563 | 0.79/0.82 | 0.61/0.65 | 0.536 | 0.109 |
| **13** | 11 fine-tuned on v3.6 with 1.5x water weight (final) | 32 | 0.40 | fine-tune | 0.657 | 0.724/0.646 | 0.90/0.95 | 0.76/0.65 | 0.617 | 0.109 |
...
11 (data cleaning + architecture-optimization retrain) became the strongest overall four-class model in the late stage, but its water recall on the fresh test was only 0.517. 13 is 11 fine-tuned on the water-augmented v3.6 dataset (89 real FSD50K water clips) at a low learning rate with a 1.5x water-class weight: fresh water recall rises to 0.650 and the 58-clip real-water holdout recall to 0.724, while scream recall stays at 0.950 and accuracy at 0.657 (11: 0.663). The joint recall at threshold 0.4 reaches 0.633, the highest among precision-controlled versions, and the probe false-positive rate remains 0.109.

Compared with 11, 13 gains 13 points of fresh water recall and 9 points of real-water recall at almost no cost (scream recall unchanged at 0.950, accuracy -0.6 pt, macro precision/recall 0.724/0.646 vs 0.785/0.672, probe FP 0.109 vs 0.063). Compared with the fusion 12, 13 is a single checkpoint, simpler to deploy, with comparable joint recall (0.633 vs 0.630) and higher scream precision (0.905 vs 0.786). 07 still leads the real-water holdout (0.776) and 09 pure joint recall (0.780 with FP 0.383), but 13 is the only single model simultaneously strong in scream, water, joint recall, and false-alarm control - the balance required by the joint drowning alarm.

11 looks stronger on the table because the macro metrics average the four classes equally, whereas the joint alarm does not: a missed water detection is the safety-critical half of the alarm, and 11's water recall of 0.52 means roughly half of the fresh water sounds are missed. 13 raises water recall to 0.65 (0.724 on the 58-clip real-water holdout) and the joint recall from 0.491 to 0.617, at the cost of some macro precision/accuracy and a higher probe false-positive rate (0.063 to 0.109). For the drowning-alert application, this is the better trade.

| Metric | 11 | 13 | Better |
|---|---:|---:|---|
| Accuracy | 0.663 | 0.657 | 11 |
| Macro precision / recall | 0.785 / 0.672 | 0.724 / 0.646 | 11 |
| Scream precision / recall | 0.97 / 0.95 | 0.90 / 0.95 | 11 (precision) |
| Water precision / recall | 0.84 / 0.52 | 0.76 / 0.65 | 11 (precision) / 13 (recall) |
| Joint recall | 0.491 | 0.617 | 13 |
